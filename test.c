...
static void test_ctr(const char* xcrypt, uint8_t* buf, size_t buf_len);
...
int main(void)
{
    ...
    uint8_t plain[32] = {0x60, 0x3d, 0xeb, 0x10, 0x15, 0xca, 0x71, 0xbe,
                        0x2b, 0x73, 0xae, 0xf0, 0x85, 0x7d, 0x77, 0x81,
                        0x1f, 0x35, 0x2c, 0x07, 0x3b, 0x61, 0x08, 0xd7,
                        0x2d, 0x98, 0x10, 0xa3, 0x09, 0x14, 0xdf, 0xf4};
    uint8_t buf[32];
    memcpy(buf, plain, 32);
    test_ctr("Encrypt", buf, 32);
    test_ctr("Decrypt", buf, 32);
...
static void test_ctr(const char* xcrypt, uint8_t* buf, size_t buf_len) {
  ...
  printf("TEST_CTR %s input:\n", xcrypt);
  phex(buf);
  AES_init_ctx_iv(&ctx, key, iv);
  AES_CTR_xcrypt_buffer(&ctx, buf, buf_len);
  printf("TEST_CTR %s output:\n", xcrypt);
  phex(buf);
}
